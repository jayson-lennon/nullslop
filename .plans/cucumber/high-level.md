...
- All cucumber tests pass (`cargo test -p nullslop-cucumber`).
...
- [ ] Phase 1: Delete redundant tui/app.rs tests + convert provider/switch_handler.rs
  ...
  - Create `tests/nullslop-cucumber/tests/features/bus/provider_switch.feature` covering all 6 scenarios: valid switch, emits switched event, rejects unknown, rejects unavailable, handles remote model, rejects unknown remote
  - Delete `#[cfg(test)]` block from `crates/nullslop-component/src/provider/switch_handler.rs`
  - Verify: `cargo test -p nullslop-cucumber --test bus_cucumber` and `just test`
...
- [ ] Phase 2: Convert provider/refresh_handler.rs and chat_log/handler.rs
  ...
  - Create `tests/nullslop-cucumber/tests/features/bus/provider_refresh.feature` covering all 6 scenarios from refresh_handler.rs
  ...
  - Create `tests/nullslop-cucumber/tests/features/bus/chat_log.feature` covering all 5 scenarios from chat_log/handler.rs
...
- [ ] Phase 3: Convert shutdown_tracker/handler.rs and headless integration tests
  ...
  - Create `tests/nullslop-cucumber/tests/features/bus/shutdown_tracker.feature` covering all 4 scenarios from shutdown_tracker/handler.rs
  - Add TuiWorld step definitions for running a keystroke script and asserting quit state
  - Create `tests/nullslop-cucumber/tests/features/tui/headless_script.feature` covering the 2 integration scenarios from headless.rs (`run_script_sets_should_quit`, `run_script_is_noop_for_empty_content`)
  ...
  - Verify: `cargo test -p nullslop-cucumber` and `just test`
